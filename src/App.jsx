...
const SCORE_COMPETENCIES = [
  { label: "Pesquisa com Usuários", column: "Pesquisa com Usuários (entrevistas, teste de usabilidade etc)", commentColumn: "Comentário - Descreva suas experiências e desafios nessa área" },
  { label: "Design de Interfaces", column: "Design de Interfaces (criação de wireframes, protótipos, princípios de design, layouts etc)", commentColumn: "Comentário - Cite ferramentas utilizadas e exemplos de projetos" },
  { label: "Arquitetura de Informação", column: "Arquitetura de Informação", commentColumn: "Comentário - Descreva como organiza e estrutura a informação em seus projetos" },
  { label: "Ferramentas de Design", column: "Ferramentas de Design (Figma, Sketch, Adobe XD, Canvas etc)", commentColumn: "Comentário - Liste as ferramentas que você utiliza e o nível de familiaridade" },
  { label: "Métricas e Dados de UX", column: "Análise, Métricas e Dados de UX (GA, Hotjar, DataBricks etc)", commentColumn: "Comentário - Explique seu conhecimento em análise de interpretação de Dados e liste as ferramentas que tem familiaridade" },
  { label: "Documentação e Entregas de UX", column: "Documentação, Comunicação e Entregas de UX (relatórios, handoff, apresentações etc)", commentColumn: "Comentário - Compartilhe exemplos de como documenta e apresenta suas entregas" },
  { label: "Feedback e Iteração", column: "Feedback e Iteração nas Entregas", commentColumn: "Comentário - Como você incorpora feedbacks e realiza iterações" },
  { label: "Comunicação e Colaboração", column: "Comunicação e Colaboração em Equipe", commentColumn: "Comentário - Descreva como você se comunica e colabora em diferentes áreas, como produto e desenvolvimento" },
  { label: "Receber e Dar Feedbacks", column: "Habilidade de Receber e Dar Feedbacks", commentColumn: "Comentário - Cite situações em que você aplicou feedback construtivo e como lidou com críticas" },
  { label: "Adaptabilidade", column: "Adaptabilidade e Resolução de Problemas", commentColumn: "Comentário - Relate uma experiência onde precisou adaptar sua abordagem para resolver um desafio" },
  { label: "Autonomia", column: "Autonomia no Desenvolvimento de Projetos", commentColumn: "Comentário - Descreve como toma decisões e assume a responsabilidade em sua atuação" },
  { label: "Apresentação para Stakeholders", column: "Você se sente confortável apresentando projetos para stakeholders?" },
];
...
const TEXT_COMPETENCIES = [
  {
    ...
    mapping: { "Nunca usei": 1, "Uso básico": 2, "Uso no dia a dia": 3, "Sou referência técnica no time": 5 },
  },
  {
    label: "Design System",
...
function MetricCard({ icon, label, value }) {
  return (
    <Card className="bg-[#111111] border border-[#2A2A2A] rounded-2xl shadow-xl">
...
export default function App() {
    ...
    try {
      ...
      const parsedProfessionals = rows.slice(0, MAX_PROFESSIONALS).map((row, index) => {
        ...
        SCORE_COMPETENCIES.forEach((item) => {
          ...
          if (item.commentColumn) comments[item.label] = normalizeText(getValue(row, item.commentColumn));
        });
        ...
        return { name, role, seniority, experience, education, scores, comments, openAnswers, average, strengths, gaps };
      });
  ...
  const teamAverage = useMemo(() => {
    ...
    return competencies.map((competency) => {
      const validScores = professionals.map((p) => p.scores[competency]).filter((score) => typeof score === "number");
      const average = validScores.length ? validScores.reduce((sum, score) => sum + score, 0) / validScores.length : 0;
      ...
      return {
        ...
        dispersion: Number((max - min).toFixed(2)),
      ...
      <main className="min-h-screen bg-[#080808] text-[#F2F2F2] p-4 md:p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {professionals.length > 0 && (
            <>
              <div id="assessment-full-report" className="space-y-6">
                <section className="grid gap-4 md:grid-cols-5">
                  <MetricCard icon={<Users />} label="Profissionais" value={professionals.length} />
                  <MetricCard icon={<TrendingUp />} label="Média geral" value={overallTeamAverage} />
                  <MetricCard icon={<BarChart3 />} label="Maturidade" value={maturity} />
                  <MetricCard icon={<Award />} label="Pontos fortes" value={strengths.length} />
                  <MetricCard icon={<AlertTriangle />} label="Pontos de atenção" value={weaknesses.length} />
                </section>
              ...
              <div className="flex justify-center mt-8 gap-4 flex-wrap">
                <button
                  ...
                  className="bg-[#B43108] hover:bg-[#8A1F11] disabled:opacity-60 disabled:cursor-not-allowed transition px-8 py-4 rounded-2xl font-semibold text-[#F2F2F2] shadow-2xl text-lg"
                >
            ...
            <Card className="bg-[#111111] border border-[#2A2A2A] rounded-2xl shadow-xl">
              <CardContent className="p-8 text-center text-[#CFCFCF]">
                ...
                <p className="mt-2 text-sm text-[#7A7A7A]">
                  Escala: 1 = Não conheço | 2 = Conheço mas nunca usei | 3 = Uso mas preciso de ajuda | 4 = Uso sem dificuldades | 5 = Sou expert e referência no time.
                </p>
              </CardContent>
            </Card>
          )}
